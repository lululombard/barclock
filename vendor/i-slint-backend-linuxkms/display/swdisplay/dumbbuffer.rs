...
// barclock patch: strict double buffering instead of the upstream triple buffering.
// With three buffers the buffer handed to the renderer is always three presents old, which the
// software renderer treats as "unknown content" and repaints the whole frame every time. With two
// buffers and a wait for the previous page flip *before* rendering, the back buffer is exactly two
// frames old, the renderer's SwappedBuffers dirty tracking applies, and only the changed region is
// repainted (a ticking clock or a dragged slider instead of 1920x480 pixels).
pub struct DumbBufferDisplay {
    drm_output: DrmOutput,
    /// Currently displayed buffer (or on its way to the display)
...
impl DumbBufferDisplay {
    ...
    pub fn new(
        ...
        Ok(Arc::new(Self { drm_output, front_buffer, back_buffer }))
    }
}
...
impl super::SoftwareBufferDisplay for DumbBufferDisplay {
    ...
    fn map_back_buffer(
        ...
        // The previous flip must have landed before we draw into the buffer it replaced.
...
impl crate::display::Presenter for DumbBufferDisplay {
    fn present(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        // No wait needed here: map_back_buffer() already waited for the previous flip.
        self.back_buffer.swap(&self.front_buffer);

        // The buffer just rendered is now the front buffer (age 1 = holds the newest frame); the
        // other one holds the frame before that (age 2), which is what SwappedBuffers expects.
